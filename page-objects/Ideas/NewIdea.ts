...
export class NewIdea {
    ...
    constructor(page: Page) {
        ...
        this.ifType = page.locator('input[data-id="dosp_ideatypeid.fieldControl-LookupResultsDropdown_dosp_ideatypeid_textInputBox_with_filter_new"]')
        this.ifOpenLevel = page.locator('select[data-id="dosp_openlevel.fieldControl-option-set-select"]')
        this.ifBusinessUnit = page.getByLabel('Business Unit')
        this.ifLeadKnowledgeGroup = page.getByLabel('Lead Knowledge Group, Lookup', { exact: true })
        this.ifFundingChannel = page.getByLabel('Funding Channel, Lookup', { exact: true })
        this.ifDetailedFundingChannel = page.getByLabel('Detailed Funding Channel, Lookup', { exact: true })
    ...
    async createNewIdea() {
        ...
        await this.page.locator('span').getByText(data['fundingChannel'], { exact: true }).click()
        await this.ifDetailedFundingChannel.fill(data['detailedFundingChannel'])
        await this.page.locator('span').getByText(data['detailedFundingChannel']).click()

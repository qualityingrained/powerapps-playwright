...
export class NewAccount {
	...
	constructor(page: Page) {
		...
		this.btnfindAccountsRefresh = page
			.frameLocator("#WebResource_SearchCenter")
			.getByRole("img");
		this.findAccountsAllResults = page
			.frameLocator("#WebResource_SearchCenter")
			.locator("div.row");
		this.findAccountsConfirmationPopUp = page.locator(
			'div[data-id="confirmdialog"]',
		);
		this.enterpriseVAT = "";
	...
	async searchForEnterprise(enterpriseName: string) {
		...
		if (enterpriseName == "Belfius") {
			await this.page.route(
				"*/**/api/enduser/search/adress",
				async (route) => {
					await route.fulfill({
						path: "./page-objects/Accounts/apiResp-Belfius.json",
					});
				},
			);
		}
	...
	async selectEnterpriseFromFindAccounts() {
		...
		var regexExpr;
		const existingEnterpriseButton = await this.findAccountsAllResults
			.getByRole("img", { name: "Open" })
			.all();
		if (existingEnterpriseButton) {
			...
			regexExpr =
				/([A-Za-z]+), ([0-9]{4}) ([A-Za-z- .']+), ([A-Za-z0-9- .']*)/;
		}

		// capture the address
		const fullAddress = await selectedEnterpriseLocator
			.locator(".form > div:nth-child(2)")
			.first()
			.innerText();
		...
		if (fullAddress) {
			...
			if (matches) {
				...
				console.log(
					"selectEnterpriseFromFindAccounts: unable to capture address details: " +
						fullAddress,
				);
			}
		} else {
			console.log(
				"selectedEnterpriseFromFindAccounts: unable to read fullAddress " +
					fullAddress,
			);
		}

		// capture the selected full name
		this.enterpriseName = await selectedEnterpriseLocator
			.locator('div[class="field bigger name"]')
			.innerText();
		...
		if (existingEnterprise == false) {
			...
			await expect(
				this.page.getByText("Name: " + this.enterpriseName),
			).toHaveCount(1);
			const addressLine =
				"Address: " +
				this.enterpriseCountry +
			...
			const vatLine = await this.findAccountsConfirmationPopUp
				.locator("p#dialogMessageTextLine6_3")
				.innerText();
			...
			if (vatMatches) {
				...
				console.log(
					"selectEnterpriseFromFindAccounts: unable to capture VAT: " +
						vatLine,
				);
			}

			await this.findAccountsConfirmationPopUp
				.locator('button[data-id="confirmButton"]')
				.click();
		}
	...
	async assertEnterpriseDetails() {
		...
		expect
			.soft(await this.ifOfficialName.innerText())
			.toBe(this.enterpriseName);
		...
		expect
			...
			.toBe(this.enterprisePostalcode);
		expect
			.soft(
				await this.page
					.getByLabel("Main Address")
			...
			.toBe(this.enterpriseCity);

		expect
			.soft(
				await this.page
					.getByLabel("Official Address")
					.getByLabel("Country")
					.innerText(),
			)
			.toBe(this.enterpriseCountry);
